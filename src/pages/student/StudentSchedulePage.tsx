...
export default function StudentSchedulePage() {
  const { currentUser, students, batches, teachers } = useAppStore()
                    ...
                    return (
                      <div key={batch.id} className="plato-card p-3 flex items-center gap-3" style={{ borderLeft: `3px solid ${color}` }}>
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-[13px] font-semibold text-white/85 truncate">{batch.subject}</p>
                          {teacher && <p className="text-[11px] text-white/40">{teacher.name}</p>}
                          {batch.room && (
                            <p className="text-[10px] text-white/30 flex items-center gap-1"><MapPin size={10} /> {batch.room}</p>
                          )}
                        </div>
                        <span className="px-2 py-0.5 rounded text-[10px] font-semibold flex-shrink-0" style={{ background: `${color}15`, color }}>{batch.curriculum}</span>
                      </div>
                    )
                  })}
                </div>
                    ...
                    classes.map(({ batch, slot }) => {
                      ...
                      return (
                        <div key={batch.id} className="rounded-xl p-3 space-y-1" style={{ background: `${color}0C`, border: `1px solid ${color}25` }}>
                          ...
                          <p className="text-[11px] font-semibold text-white/80 leading-tight">{batch.subject}</p>
                          {teacher && <p className="text-[10px] text-white/35 truncate">{teacher.name}</p>}
                          {batch.room && <p className="text-[10px] text-white/25 flex items-center gap-1"><MapPin size={9} /> {batch.room}</p>}
                        </div>
                      )
                    })
                  )}
