...
export default function StudentFeesPage() {
  const { currentUser, students, invoices, branches } = useAppStore()
  ...
  const outstanding = myInvoices.filter((i) => i.status === 'pending' || i.status === 'overdue' || i.status === 'partial').reduce((sum, i) => sum + i.totalAmount, 0)
  ...
  return (
    <div className="space-y-5">
      ...
      {outstanding > 0 && (
        ...
        >
          ...
          <p className="text-[12px] text-[#FBBF24]">
            {formatCurrency(outstanding, currency)} outstanding. This page is read-only — contact your branch finance team to make a payment.
          </p>
        </div>
      )}
      ...
      {/* Branch finance contact */}
      {branch && (
        <div className="plato-card p-4 flex flex-wrap items-center gap-4">
          <p className="text-[12px] text-white/40">For payment queries, contact your branch:</p>
          <span className="text-[12px] font-semibold text-white/70">{branch.name}</span>
          <a href={`tel:${branch.phone}`} className="flex items-center gap-1.5 text-[12px] text-[#4D7CFF] hover:underline">
            <Phone size={12} /> {branch.phone}
          </a>
          <a href={`mailto:${branch.email}`} className="flex items-center gap-1.5 text-[12px] text-[#4D7CFF] hover:underline">
